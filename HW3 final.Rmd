...
mod_bk <- glm(formula = target ~ zn + nox + age + dis + rad + tax + ptratio + 
    medv, family = binomial, data = crm_df)
...
df.predicted = rbind(mod1_predict, mod2_predict, mod3_predict, probit_back_predict)

ggplot(data = df.predicted, aes(predicted.values)) + geom_histogram(color = "white", fill = "lightgreen") + facet_wrap(~model)
```
